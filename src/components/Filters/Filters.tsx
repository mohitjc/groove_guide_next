...
function Filters({
  ...
  const [theraCategories, setTheraCategories] = useState([]);
  ...
  useEffect(() => {
    if (tab == 'therapeutic' || tab == 'functional_medicinal') {
      const value = tab == 'therapeutic' ? 'Therapeutic Use' : 'Health & Wellness'
  ...
  const tabs = [
    { id: 'my_groove', name: 'My Groove' },
    // {id:'groove_deals',name:'Groove Deals'},
    { id: 'therapeutic', name: 'Therapeutic' },
    { id: 'functional_medicinal', name: 'Functional/Medicinal' },
    { id: 'experiences', name: 'Experiences' },
    { id: 'health_benefits', name: 'Health Benefits' },
  ]
  ...
  const catClick = (itm:any) => {
    const id = itm.id
    let arr:any = selectedCatObj || []
    if (arr.map((itm:any) => itm.id).includes(id)) {
      arr = arr.filter((itm:any) => itm.id != id)
    } else {
      arr.push(itm)
    }
            ...
            <>
              <div className={`flex items-center !justify-start md:!justify-between gap-4 mb-0 border-gray-200 md:border-none md:pb-0  `}>
                <div className="hidden max-md:flex  items-center gap-2">
                  <Image src="/assets/img/filter.svg" alt={""} width={50} height={50}/>
                ...
                {(selectedTag?.length || searchValue || filters?.categories?.length || grooves || selectedDeals?.length) ? (
                  <a className="ml-auto" onClick={onClearAll}>
                    <h6 className="text-black font-semibold text-[12px] xl:text-[15px]">
                      Clear all
                    </h6>
                  </a>
                ) : <></>}
          ...
          <div className="block md:hidden pt-4">
            ...
            <div className="flex gap-2">
              ...
              <div onClick={() => setIsOpen3(true)} className="bg-[#F9F7F5] border border-[#E8E8EA] cursor-pointer flex items-center justify-center px-4 py-1 rounded-lg">
                <Image width={200} height={200} alt="filter" src="/assets/img/v2/filter.svg" className="h-8" />
              </div>
            </div>
